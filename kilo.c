...
void die(const char *s){ 	//Mostrar o erro mais Bonito
...
void enableRawMode(){
	if(tcgetattr(STDIN_FILENO, &E.orig_termios) == -1) die("tchetattr"); //Fazendo um Backup das COnfig do Terminal
	atexit(disableRawMode);

	struct termios raw = E.orig_termios; //Cria uma Struct do Tipo termios
...
void editorRefreshScreen(){
	...
	write(STDOUT_FILENO, "\x1b[H", 3);//Seta o Cursor de Volta para o Inicio
}
